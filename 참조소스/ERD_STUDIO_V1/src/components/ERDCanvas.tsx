...
const ERDCanvas: React.FC = () => {
  ...
  const {
    ...
    addRelation, searchTerm, selection, setContextMenu
  } = useSchemaStore();
  ...
  validationIssues.forEach(issue => {
    if (issue.tableId) {
      const arr = issuesByTable.get(issue.tableId) || [];
      arr.push(issue);
      issuesByTable.set(issue.tableId, arr);
    }
  });
  ...
  if (hoveredRel) {
    const rel = view.relations.find(r => `${r.from}-${r.to}` === hoveredRel);
    if (rel) {
      relatedTableIds.add(rel.from);
      relatedTableIds.add(rel.to);
    }
  }
  ...
  const handleCanvasMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (isRelDragging && relFrom && workspaceRef.current) {
      ...
      if (fromElem) {
        ...
        const x1 = (fromRect.left + fromRect.width / 2 - canvasRect.left) / viewState.scale;
        const y1 = (fromRect.top + fromRect.height / 2 - canvasRect.top) / viewState.scale;
  ...
  const handleCanvasMouseUp = useCallback((e: React.MouseEvent) => {
    ...
    if (isRelDragging && relFrom) {
      ...
      const colMatch = target.closest('[id^="col-"]') as HTMLElement;
      if (colMatch) {
        ...
        if (toTableId !== relFrom.tableId) {
          addRelation({
            from: relFrom.tableId,
            to: toTableId,
            fromCol: relFrom.colName,
            toCol: toColName,
            identifying: !e.shiftKey,
            cardinality: '1 : N'
          });
        }
      }
  ...
  const connections = view.relations.map(rel => {
    ...
    if (Math.abs(cardDy) > Math.abs(cardDx) * 1.2) {
      ...
      if (cardDy > 0) {
        y1 = (fromRect.bottom - canvasRect.top) / viewState.scale + offset;
        y2 = (toRect.top - canvasRect.top) / viewState.scale - offset;
      } else {
        y1 = (fromRect.top - canvasRect.top) / viewState.scale - offset;
        y2 = (toRect.bottom - canvasRect.top) / viewState.scale + offset;
      }
    } else {
      type = 'horizontal';
      ...
      let rawX1 = (fromRect.right - canvasRect.left) / viewState.scale;
      let rawX2 = (toRect.left - canvasRect.left) / viewState.scale;
      if (rawX1 < rawX2) {
        x1 = rawX1 + offset;
        x2 = rawX2 - offset;
        direction = 1;
    ...
    const isDimmed = searchTerm.length > 0 && !isHighlighted;

    return { rel, x1, y1, x2, y2, type, direction, isHighlighted, isDimmed };
  }).filter(Boolean);
    ...
    >
      ...
      >
        <svg className="connections-svg" style={{ width: '100%', height: '100%' }}>
          <defs>
            <marker id="marker-arrow" viewBox="0 0 12 12" refX="10" refY="6" markerWidth="10" markerHeight="10" orient="auto-start-reverse">
              <path d="M 2,2 L 10,6 L 2,10 Z" fill={accentColor} stroke={accentColor} strokeWidth="1.5" strokeLinejoin="round" />
            </marker>
          </defs>
        ...
        {view.tables.map(table => {
          ...
          const isDimmed = searchTerm.length > 0 && !isMatched && !matchedTableIds.some(id => {
            const rels = view.relations.filter(r => r.from === id || r.to === id);
            return rels.some(r => r.from === table.id || r.to === table.id);
          });
          ...
          return (
            <React.Fragment key={table.id}>
              <EntityCard
                ...
                onContextMenu={(e) => showMenu(e, 'table', table.id)}
              />
              {tableIssues.length > 0 && (
                <div
                  className={`validation-badge ${tableIssues[0].type}`}
                  style={{ left: table.x + 280, top: table.y - 8 }}
                >
                  !
                </div>
              )}
      ...
      {selection.isAreaSelecting && selection.areaStart && selection.areaEnd && (
        <div
