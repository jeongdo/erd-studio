...
(() => {
  'use strict';
  const E=window.ERDEditor, R=E?.RelationRouting, I=E?.RelationIdentity, V2=E?.RelationRouterV2, S=E?.RelationRouteStrategies;
  const Actions=E?.Actions, A=E?.Advanced, base=window.updateConnections;
  if(!E||!R||!I||!V2||!S||typeof base!=='function')return;
  ...
  function directRoute(rel,index,relations,canvasLayer,scale){
    ...
    const raw=R.computeRoute({fromColumn,toColumn,fromCard,toCard,canvas,scale});
    const lane=I.parallelLane?.(rel,index,relations)||0;
    const route=I.laneRoute?.(raw,lane)||raw;
    return {...route,lane,canvas};
  }
  ...
  function refine(){
    ...
    const scale=R.readCanvasScale?.(canvasLayer)||1, routed=[];
    document.querySelectorAll('#connections-svg .connection-line').forEach(path=>{
      ...
      const direct=directRoute(rel,index,relations,canvasLayer,scale), cubic=direct&&V2.parseCubic(direct.d);if(!direct||!cubic)return;
      const obs=obstaclesFor(rel.from,rel.to,direct.canvas,scale), key=relationKey(rel,index), previous=history.get(key)||'';
